...
export function useCreateManySession(options?: MutationOptions<Prisma.BatchPayload, unknown, Prisma.SessionCreateManyArgs>) {
    const mutation = request.useModelMutation('Session', 'POST', 'createMany', metadata, options, false);
    return {
        ...mutation,
        trigger: <T extends Prisma.SessionCreateManyArgs>(args: Prisma.SelectSubset<T, Prisma.SessionCreateManyArgs>) => {
...
export function useCheckSession(args: { operation: PolicyCrudKind; where?: { id?: string; sessionToken?: string; userId?: string }; }, options?: QueryOptions<boolean>) {
    return request.useModelQuery('Session', 'check', args, options);
}
